...
def get_columns(filters):
	...
	if filters.get("summary_report"):
		...
		return [
			...
			_("Debit") + ":Float:100", _("Debit before Rebate") + ":Float:100",
			_("Credit") + ":Float:100",
			_("Balance") + ":Float:100", _("Balance before Rebate") + ":Float:100", 						
...
def get_gl_entries(filters):
	...
	select
		...
		sum(debit) as debit,
			...
			THEN
				sum((select consoleerp_customer_discount_total from `tabSales Invoice`
				where name=voucher_no)+debit)
			ELSE
				sum(debit) END AS debit_before_rebate,
		...
		for party_type in party_types:			
			for party, party_dict in gle_map[party_type].items():
				if party_dict.entries:
					...
					total_debit_before_rebate = (party_dict.total_debit_before_rebate + party_dict.opening_before_rebate) \
													if party_dict.opening_before_rebate > 0 else party_dict.total_debit_before_rebate
					total_credit = party_dict.total_credit +(-1 * party_dict.opening) if party_dict.opening < 0 else party_dict.total_credit
					#opening row
					data.append(get_balance_row(_("Opening"), party_dict))
					data += party_dict.entries
					data += [{
							'party': "'" +_("Totals") + "'", 
							"debit": total_debit,
							"debit_before_rebate": total_debit_before_rebate,
							"credit": total_credit,
							"balance": total_debit - total_credit,
							"balance_before_rebate": total_debit_before_rebate - total_credit
							},	{}]
...
def initialize_gle_map(gl_entries):
	...
	for gle in gl_entries:
		gle_map[gle.party_type].setdefault(gle.party, frappe._dict({
					"opening": 0,
					"opening_before_rebate": 0,
					"entries": [],
					"total_debit": 0,
					"total_debit_before_rebate": 0,
					"total_credit": 0,
					"closing": 0				
		}))
	return gle_map
...
def get_partywise_gle(filters, gl_entries, gle_map):
	...
	for gle in gl_entries:
		...
		balance_before_rebate = flt(gle.debit_before_rebate, 3) - flt(gle.credit, 3)
		# add balance to the gle_row
		gle["balance"] = balance
		
		if gle.posting_date < from_date or cstr(gle.is_opening) == "Yes":
			gle_map[gle.party_type][gle.party].opening += balance
			gle_map[gle.party_type][gle.party].opening_before_rebate += balance_before_rebate
			...
			party_dict.total_debit_before_rebate += flt(gle.debit_before_rebate, 3)
			party_dict.total_credit += flt(gle.credit, 3)
			
	return gle_map
	
def get_balance_row(label, party_dict):
	balance = party_dict.opening
	return {
		"party": "'" + label + "'",
		"debit": balance if balance > 0 else 0,
		"debit_before_rebate": party_dict.opening_before_rebate if party_dict.opening_before_rebate > 0 else 0,
		"credit": -1*balance if balance < 0 else 0,
		"balance": balance,
		"balance_before_rebate": party_dict.opening_before_rebate
	}
	...
	if filters.get("summary_report"):
		...
		for d in data:
			result.append([
				...
				d.get("party"),
				d.get("debit"), d.get("debit_before_rebate"),
				d.get("credit"),
				d.get("balance"), d.get("balance_before_rebate"),
